...
SupplierLedgerEntry representing a sub ledger entry on a [Supplier](GoApi.Party.Supplier.md)
...
[IsCreatedFromEhf](GoApi.Reporting.Ledger.SupplierLedgerEntry.IsCreatedFromEhf.md)|Gets or sets a value indicating whether this instance is created from ehf.         Note: This property cannot be used in odata filtering.
...
[IsWriteOff](GoApi.Reporting.Ledger.SupplierLedgerEntry.IsWriteOff.md)|Gets a flag indicating whether the entry is created by the write off functionality in Go. Such entries are not payments, but an accounting routine writing off an open item.
...
[Supplier](GoApi.Reporting.Ledger.SupplierLedgerEntry.Supplier.md)|OBSOLETE - Gets or sets the supplier.         Note: Returned object only contains Id, Code and Name of Supplier, all other properties are empty (SupplierCreatedDate, CreatedDate and LastChanged are default value since they are not nullable).         Use Supplier.Id (or SupplierCode) and query the Supplier endpoint to get full Supplier object.
[SupplierCode](GoApi.Reporting.Ledger.SupplierLedgerEntry.SupplierCode.md)|Gets the code of the GoApi.Reporting.Ledger.SupplierLedgerEntry.Supplier , used for filtering since OData does not support filtering by sub-data objects like [Supplier](GoApi.Party.Supplier.md) code.
...
Name | Description
:----|:------------
[SupplierLedgerEntry()](GoApi.Reporting.Ledger.SupplierLedgerEntry.SupplierLedgerEntry__.md)|
