...
GET Supplier/{parentId}/BankAccount/{id}?options=&lt;options&gt;
================
...
### Url
```http
https://api.poweroffice.net/Supplier/{parentId}/BankAccount/{id}?options=<options>
```
...
### Request Signature
```http
GET /Supplier/{parentId}/BankAccount/{id}?options=<options> HTTP/1.1
