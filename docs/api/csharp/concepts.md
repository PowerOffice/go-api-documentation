...
api.Customer.Get()
	...
	.Take(50);
```
If you only want a single object, we highly recommend that you use **SingleOrDefault** instead of FirstOrDefault, as this will execute faster on our servers, and will reduce database load.

```csharp
api.Customer.Get().SingleOrDefault(x => x.Code == 10000)
```